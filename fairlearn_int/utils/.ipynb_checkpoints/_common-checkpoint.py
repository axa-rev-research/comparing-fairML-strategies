...
def _get_soft_predictions(estimator, X, y, Z, predict_method, bld=None):
    ...
    if bld==None:
        output = estimator(torch.Tensor(X.to_numpy())).detach().numpy()
        #output = getattr(estimator, predict_method)(X, y, Z)
    else:
        output = estimator.predict(bld).scores# getattr(estimator, predict_method)(bld)
        #col = output.label_names
        #output = output.convert_to_dataframe()[0][col]
