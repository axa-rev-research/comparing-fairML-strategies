...
class InterpolatedThresholder(BaseEstimator, MetaEstimatorMixin):
    ...
    def _pmf_predict(self, X, y, Z, sensitive_features, bld):
        ...
        base_predictions = np.array(
            _get_soft_predictions(self.estimator_, X, y, Z, self._predict_method, bld)
        )
        (
            _,
    ...
    def predict(self, X, y, Z, sensitive_features, random_state=None, bld=None):
        ...
        positive_probs = self._pmf_predict(X, y, Z, sensitive_features=sensitive_features, bld=bld)[
            :, 1
        ]
